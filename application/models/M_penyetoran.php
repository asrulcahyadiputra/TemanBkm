...
class M_penyetoran extends CI_Model
{
	...
	public function id_simpanan()
	{
		...
		$kodetampil = "TRX-PNY" . "-" . $date . '-' . $batas;  //format kode
		return $kodetampil;
	}
	private function _rekening($rek)
	{
		$this->db->select('rekening.no_rekening,anggota.nama_anggota,sum(if(ket="dr",detail_rekening.nominal,0)) as s_debet, sum(if(ket="cr",detail_rekening.nominal,0)) as s_kredit,p_simpanan.nama_simpanan,rekening.status,rekening.tgl_pembuatan,p_simpanan.saldo_minimal,p_simpanan.penarikan,p_simpanan.setor')
	...
	public function store()
	{
		...
		$minimal = intval(preg_replace("/[^0-9]/", "", $this->input->post('setoran_minimal')));
		...
		if ($nominal < $minimal) {
			...
			if ($rekening['setor'] == 0) {
				$this->session->set_flashdata('sukses', 'warning5');
				redirect('Penyetoran');
			} else {
				$simpanan = [
				...
				$this->session->set_flashdata('sukses', 'Penyetoran Berhasil');
				redirect('Penyetoran');
			}
		}
	}
